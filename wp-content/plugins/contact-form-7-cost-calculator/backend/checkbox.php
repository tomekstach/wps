...
function wpcf7_add_form_tag_checkbox_custom() {
	wpcf7_add_form_tag( array( 'checkbox_custom', 'checkbox_custom*', 'radio_custom' ),
...
function wpcf7_checkbox_custom_posted_data( $posted_data ) {
	$tags = wpcf7_scan_form_tags(
		array( 'type' => array( 'checkbox_custom', 'checkbox_custom*', 'radio_custom' ) ) );
	...
	foreach ( $tags as $tag ) {
		...
		$posted_data[$tag->name] = implode(",", $posted_items);
	}
...
function wpcf7_add_tag_generator_checkbox_and_radio_custom() {
	...
	$tag_generator->add( 'checkbox_custom', __( 'checkboxes price', 'contact-form-7' ),
		'wpcf7_tag_generator_checkbox_custom' );
	$tag_generator->add( 'radio_custom', __( 'radio buttons price', 'contact-form-7' ),
		'wpcf7_tag_generator_checkbox_custom' );
}
...
<?php endif; ?>
	...
	<td>
		...
		<?php echo  __( "One option per line (number|text): Ex: <strong>10|Blue $10</strong>", 'contact-form-7'  ); ?></span></label><br />
