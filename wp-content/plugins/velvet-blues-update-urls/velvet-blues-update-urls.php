...
function VelvetBluesUU_management_page(){
	if ( !function_exists( 'VB_update_urls' ) ) {
		function VB_update_urls($options,$oldurl,$newurl,$start){	
			global $wpdb;
			$results = array();
			$queries = array(
			...
			foreach($options as $option){
				if( $option == 'custom' ){
					$n = 0;
					$row_count = $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_value LIKE '%$oldurl%'" );
					$page_size = 100;
          $pages = ceil( $row_count / $page_size );
            ...
            if ($row_count < $page_size) {
              $page_size = $row_count;
						...
						$current_row = 0;
            $pmquery = "SELECT * FROM $wpdb->postmeta WHERE meta_value LIKE '%$oldurl%' ORDER BY meta_id  LIMIT $start, $page_size";
            $items = $wpdb->get_results( $pmquery );
              ...
              $raw_data = get_metadata( 'post', $item->post_id, $item->meta_key );
              $n++;
						
              $edited = VB_replace( $oldurl, $newurl, $raw_data );
						
              $data = slash_settings( $edited )[0];
              update_metadata( 'post', $item->post_id, $item->meta_key, $data );
						}
					}
					$results[$option] = array($n.' from '.$row_count, $queries[$option][1], $row_count);
				}
				else{
	...
	if ( !function_exists( 'VB_replace' ) ) {
		function VB_replace( $from = '', $to = '', $data = '' ) {
			try {
				if ( is_array( $data ) ) {
					$_tmp = array( );
					foreach ( $data as $key => $value ) {
						$_tmp[ $key ] = VB_replace( $from, $to, $value );
					}
					$data = $_tmp;
					unset( $_tmp );
				}
	...
	elseif( isset( $_POST['VBUU_settings_submit'] ) ){
		...
		if(isset($_POST['VBUU_oldurl']) && isset($_POST['VBUU_newurl'])){
			if(function_exists('esc_attr')){
				$vbuu_oldurl = esc_attr(trim($_POST['VBUU_oldurl']));
        $vbuu_newurl = esc_attr(trim($_POST['VBUU_newurl']));
        $vbuu_start = esc_attr(trim($_POST['VBUU_start']));
			}else{
				$vbuu_oldurl = attribute_escape(trim($_POST['VBUU_oldurl']));
        $vbuu_newurl = attribute_escape(trim($_POST['VBUU_newurl']));
        $vbuu_start = attribute_escape(trim($_POST['VBUU_start']));
			}
		}
		if(($vbuu_oldurl && $vbuu_oldurl != 'http://www.oldurl.com' && trim($vbuu_oldurl) != '') && ($vbuu_newurl && $vbuu_newurl != 'http://www.newurl.com' && trim($vbuu_newurl) != '')){
			$results = VB_update_urls($vbuu_update_links,$vbuu_oldurl,$vbuu_newurl,$vbuu_start);
			$empty = true;
...
?>
        ...
        <tr valign="middle">
					<th scope="row" width="140" style="width:140px"><strong>
						...
						<span class="description">
						For the limit param in the query
						</span></th>
					<td><input name="VBUU_start" type="text" id="VBUU_start" value="<?php echo (isset($vbuu_start) && trim($vbuu_start) != '')? $vbuu_start : '0'; ?>" style="width:300px;font-size:20px;" /></td>
				</tr>
			</table>
			<br/>
			<h3 style="margin-bottom:5px;">
...
function slash_settings( $data ) {
  if ( is_array( $data ) ) {
    foreach ( $data as $key => $val ) {
      $data[ $key ] = slash_settings( $val );
    }
  } elseif ( is_object( $data ) ) {
    foreach ( $data as $key => $val ) {
      $data->$key = slash_settings( $val );
...
function clean_layout_data( $data = array() ) {
  $cleaned = array();
