...
switch ( $action ) {
	...
	<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url('wp-login.php?action=lostpassword', 'login') ); ?>" method="post">
	<p>
	...
	<p id="nav">
	<a href="<?php echo esc_url( wp_login_url().'?redirect_to='.$redirect_to ); ?>"><?php _e( 'Log in' ); ?></a>
		<?php
	...
	case 'rp':
		...
		if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
			...
			setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
			login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( wp_login_url().'?redirect_to='.$redirect_to ) . '">' . __( 'Log in' ) . '</a></p>' );
			login_footer();
			exit;
		}
	...
	<a href="<?php echo esc_url( site_url('wp-login.php?action=lostpassword&redirect_to=' . $redirect_to, 'login') ); ?>"><?php _e( 'Lost your password?' ); ?></a>
	</p>
